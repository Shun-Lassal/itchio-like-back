...
const register = async (user) => {
  ...
  const userDocument = new Model.User({ email: credentials.email, password: hashedPassword, role_fk: defaultRole, superAdmin: false });
  const resultEmail = await Repository.findUsersByAny({email:credentials.email})
...
const login = async (user) => {
  ...
  let credentials = { email: user.email, password: user.password }
  let [{_id,email,role_fk, password}]= await Repository.findUsersByAny({ email: credentials.email });
...
const changePassword = async (values) => {
  ...
  if(resultBcrypt) {
    ...
    let result = await Repository.updateAnyUserValues(user.id ,user) // on return un truc ou pas ?
